...
var inventory =
{
	...
	render: function()
	{
		...
		self.items.forEach(function(item)
		{
			...
			html += ' title="' + item.name + '">' +
				'<img src="http://steamcommunity-a.akamaihd.net/economy/image/' + item.image + '/80fx80f" alt="">' +
				'<div class="state"></div>';
		...
		$items.find('.item').click(function()
		{
			...
			{
				$self.find('.status span').text('Промежуточный инвентарь (' + self.items.length + ' из ' + self.maximumItemsCount + ') (' + self.sum + ' руб.)');
		...
		var itemWidth = $('#inventory .item').width();

		if (itemWidth * self.items.length < itemsWidth)
		{
			$('#inventory .items').css('left', '0');
		}
		else
		{
			if (self.scrollOffsetX < -(itemWidth * self.items.length - itemsWidth))
			{
				self.scrollOffsetX = -(itemWidth * self.items.length - itemsWidth);
			}
		...
		{
			$('#inventory .status span').text('Промежуточный инвентарь (' + self.items.length + ' из ' + self.maximumItemsCount + ') (' + self.sum + ' руб.)');
			$('#inventory .selectAll').hide();
			$('#inventory .moveToZone').hide();
			$('#inventory .status').show();
			$('#inventory .cancel').hide();
	...
	scroll: function(direction)
	{
		...
		self.scrollOffsetX = self.scrollOffsetX + itemWidth * 5 * direction;

		if (self.scrollOffsetX > 0)
		{
			self.scrollOffsetX = 0;
		}
		else if (self.scrollOffsetX < -(itemWidth * self.items.length - itemsWidth))
		{
			self.scrollOffsetX = -(itemWidth * self.items.length - itemsWidth);
		}
...
var router =
{
		...
		'#market/purchases': function()
		{
			...
			function(error, data)
			{
				...
				data.items.forEach(function(item, index)
				{
					...
					$item.find('.instance img').attr('src', 'http://steamcommunity-a.akamaihd.net/economy/image/' + item.image + '/100fx100f');
					...
					$item.find('.bought span').text(item.price);
		...
		'#market/sell': function()
		{
			...
			function(error, data)
			{
				...
				data.items.forEach(function(item, index)
				{
					...
					$item.find('.control .putUp.button').click(function()
					{
						...
						function(error, data)
						{
							...
							noty(
							{
								text: '<div>Ваш предмет успешно выставлен на продажу!</div>',
								type: 'success',
							});
						});
					});
				...
				if (data.items.length > 0)
				{
					$('#marketPage .sell.container table').show();
				}
			});
		},
		...
		'#market/sales': function()
		{
			...
			function(error, data)
			{
				...
				data.items.forEach(function(item, index)
				{
					...
					$item.find('.instance .quality').css('background', '#' + item.qualityColor).attr('title', item.quality);
					$item.find('.sold span').text(item.price);
					$item.find('.income span').text(Math.floor(item.price / (1.0 + item.commission) * 100) / 100);
	...
	recover: function()
	{
		...
		$('#marketPage .sell.container table').hide().find('.items').html('');
	...
	jumped: function()
	{
		...
		{
			$('#inventory .status span').text('Промежуточный инвентарь (' + inventory.items.length + ' из ' + inventory.maximumItemsCount + ') (' + inventory.sum + ' руб.)');
			$('#inventory .status').show();
			$('#inventory .moveToZone').hide();
			$('#inventory .outboundTransfer').show();
		...
		update: function()
		{
			...
			function(error, data)
			{
				...
				data.instances.forEach(function(instance, index)
				{
					...
					$instance.find('img').attr('src', 'http://steamcommunity-a.akamaihd.net/economy/image/' + instance.image + '/94fx94f');
					...
					$instance.click(function()
					{
						...
						function(error, data)
						{
							...
							data.items.forEach(function(item)
							{
								...
								if (item.category && item.category != 'Normal')
								{
									$item.find('.instance .category').css('background', '#' + item.categoryColor).attr('title', item.category);
								}
								else
								{
									$item.find('.instance .category').hide();
...
var recalculateSizes = function()
{
    ...
    if ($('#middle').innerHeight() < $(window).height())
    {
    	var middleWrapperHeight = $(window).height();
    	middleWrapperHeight -= $('#middle').innerHeight() - $('#middle').height();
    	middleWrapperHeight -= $('#middle .wrapper').innerHeight() - $('#middle .wrapper').height();
...
$(function()
{
	...
	socket.on('authorize', function(data)
	{
		...
		if (data.suitableBotA)
		{
			$('#inventory .control .makeInboundTransfer.button').attr('href', 'https://steamcommunity.com/tradeoffer/new/?partner=' + data.suitableBotA.steamId32
				+ '&token=' + data.suitableBotA.accessToken).unbind('click');
		}
		else
		{
			$('#inventory .control .makeInboundTransfer.button').attr('href', '').click(function()
			{
				noty(
				{
					text: '<div>Произошла техническая ошибка, попробуйте позднее</div>',
					type: 'error',
				});
	...
	socket.on('suitableBotA', function(suitableBotA)
	{
		...
		$('#inventory .control .makeInboundTransfer.button').attr('href', 'https://steamcommunity.com/tradeoffer/new/?partner=' + suitableBotA.steamId32
			+ '&token=' + suitableBotA.accessToken).unbind('click');
	});
	...
	$('#marketPage .side .filter').hover(null, function()
	{
		$(this).removeClass('active');
		$(this).find('ul').hide();
	});
